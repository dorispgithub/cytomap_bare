    ...
    methods (Static)
        ...
        function func_load(app, NSamp)
            ...
            if NSamp==0
                ...
                [fnm , pnm]=uigetfile({'*.mat;*.csv','Compatible Files';'*.mat', 'Matlab Workspace Files (*.mat)'; '*.csv', 'FlowJo files (*.csv)'}, ...
                    'Select either FLOWJO csv files that you want to analyze or dat.mat file you want to look at','Multiselect', 'on');
        ...
        function func_load_mat(app, fnm, current_folder)
            ...
            for f_i = 1:numel(fnm{1})
                ...
                if ~isstruct(dat.map)
                    dat.map = struct;
                    dat.map.jet = jet;
                    % Put in cases for RSN and CCN BOB
                    app.map = dat.map;
                else
                    app.map = dat.map;
                end
            ...
            for smpl_idx = 1:numel(fieldnames(app.data))
                ...
                if isfield(app.data.(smpl_i), 'Gates')
                    if ~isfield(app, 'NewGate') || isempty(fieldnames(app.NewGate))
                        app.data.(smpl_i) = rmfield(app.data.(smpl_i), 'Gates');
                    else
                        gates_i = app.data.(smpl_i).Gates;
                        app.data.(smpl_i).Gates = gates_i(ismember(gates_i, fieldnames(app.NewGate)));
                    end
                end
            end
        ...
        function func_load_csv(app, fnm, pnm, current_folder, web)
            ...
            vPD = waitbar(0, 'Initializing');
            waitbar(0.1, vPD, 'Loading data');
            if ~all(endsWith(fnm{1}, '.csv'))
                errordlg('All files have to be the same type');
                return;
            end

            SampleNames = IO.get_sample_names(app, pnm);
            ...
            t.ColumnName = { ...
                'SampleGroups', 'SampleAnnotation','SampleNames', ...
                'FileNames', 'ImportCells', 'CellNames','String to Replace', 'Replace with', 'Final Cell Names', ...
                'ChannelNames', 'ChNReplace', 'ChNReplacements', 'Final Channel Names' ...
            };
            ...
            % Create a push button
            btn = uibutton(UIfig,'push', 'ButtonPushedFcn', @(btn,event) IO.func_load_execute(UIfig, app, dat, fnm{end}, pnm, SampleNames{end}, t.Data));
...
% % %             end
        ...
        function tab_data = get_table_data(fnm, dat, SampleNames)
            ...
            for smpl_idx=1:numel(SampleNames)
                ...
                CellNames =  unique([CellNames; smpl_c_names]);
            end
        ...
        function SampleNames = get_sample_names(app, pnm)
        ...
        function dat = get_dat(pnm, fnm, SampleNames)
            ...
            for pnm_idx = 1:numel(pnm)
                ...
                for j = 1:size(fnm{pnm_idx}, 2)
                    ...
                    dat.(pnm_name).GatesTMP.(gate) = array2table(dat00.data);
                    ...
                    if ~ismember('X', dat.(pnm_name).GatesTMP.(gate).Properties.VariableNames)
                        ...
                        indx = listdlg('PromptString', 'Select the X axis:','SelectionMode','single', 'ListString', VNamestmp);
                    ...
                    if ~ismember('Y', dat.(pnm_name).GatesTMP.(gate).Properties.VariableNames)
                        redefineY = 1;
                        VNamestmp = dat.(pnm_name).GatesTMP.(gate).Properties.VariableNames;
                        indy = listdlg('PromptString', 'Select the Y axis:','SelectionMode','single', 'ListString', VNamestmp);
                    ...
                    if ~ismember('Z', dat.(pnm_name).GatesTMP.(gate).Properties.VariableNames)
                        VNamestmp = dat.(pnm_name).GatesTMP.(gate).Properties.VariableNames;
                        indz = listdlg('PromptString', 'Select the Z axis:','SelectionMode','single', 'ListString', [{'There is no Z (make a fake one, otherwise everything breaks)'},VNamestmp(:)']);
        ...
        function func_load_execute(UIfig, app, dat, fnm, pnm, DName, t_dat)
            ...
            for pnm_j = 1:numel(pnm)
                ...
                for i=2:numel(dat.(SNsOLD{pnm_j}).CNames)
                    ...
                    Logic = ismember(DatSub,dat.(SNsOLD{pnm_j}).AllCells);
                ...
                for i=1:numel(fieldnames(dat.(SNsOLD{pnm_j}).Gates))
                    DatSub = dat.(SNsOLD{pnm_j}).Gates.(dat.(SNsOLD{pnm_j}).CNames{i});
                    SubNames = DatSub.Properties.VariableNames;
                    ...
                    for ch_n=1:numel(ChNames)
                        IND(ch_n) = find(strcmp(SubNames, ChNames{ch_n}));
                    end
                    % rearange your gate to be in the same order as AllCells
                    DatSub = DatSub(:, IND);
                    % Find which cells in AllCells belong to this specific
                    % gate
                    Logic = ismember(dat.(SNsOLD{pnm_j}).AllCells,DatSub);
                    % Put a one in the column corresponding to the gate for
                    % each cell that belongs to that group
                    dat.(SNsOLD{pnm_j}).SortedDat.(dat.(SNsOLD{pnm_j}).CNames{i})  = Logic;
                end
...
% %                     treed_struct.GateTags.(sprintf([Constants.gate_tag '%d'], tag)) = strcat(main_tree.name, '/', kid.name);
        ...
        function func_ExportCSV(app, web)
            ...
            MFINMS = app.data.(app.DataN.Value).AllCells.Properties.VariableNames;
            MFINMS = MFINMS(~startsWith(MFINMS, Constants.gate_tag));

            tbldat = Helper.populate_table(app, 'smpls', {app.DataN.Value}, 'MFI', 'MFIRSN');
            tbldat = [tbldat; tbldat(end,:)];
            tbldat(:,1) = [];
            tbldat(:,1) = {false};
            tbldat(:,3) = [];
            tbldat(1:numel(MFINMS),3) = {true};
            tbldat(1:numel(MFINMS),4) = MFINMS;
            tbldat(end,4) = {'All Channels'};
            tbldat(end,3) = {false};
%             tbldat((numel(MFINMS)+1),3) = {false};
            
            tbldat(end,1) = {false};
            tbldat(end,2) = {'All Cells'};
            ...
            lbl = uilabel(UIfig);
            lbl.Text = sprintf('Regions to export (e.g.: 1,2,6):');
            lbl.FontColor = app.txtclr; lbl.FontSize = app.GUIOPTS.AlphaF{app.GUIOPTS.AlphaI};
            lbl.Position = alpha*[10+245 35 400 15];
            
            RegSel = uieditfield(UIfig,'text');
            RegSel.Value = '';
            RegSel.Position = alpha*[10+240+100, 5, 100, 30];
            RegSel.BackgroundColor = app.bgclr;

            Model = uidropdown(UIfig);
            Model.Position = alpha*[10+240 5 100 30];
            Model.BackgroundColor = app.bgclr;
            ...
            lbl = uilabel(UIfig);
            lbl.Text = sprintf('Select Neighborhood Type:');
            lbl.FontColor = app.txtclr; lbl.FontSize = app.GUIOPTS.AlphaF{app.GUIOPTS.AlphaI};
            ...
            DataType.BackgroundColor = app.bgclr;

            % Create a Neighborhood export button
            btn = uibutton(UIfig,'push', 'ButtonPushedFcn', @(btn,event) wrap_backend(t, app, DataType, RegSel, Model, false));
            btn.Position = alpha*[750-145-145, 0, 135, 45];
            btn.Text = 'Export Neighborhoods';
            btn.BackgroundColor = app.bgclr;

            % Create a push button
            btn = uibutton(UIfig,'push', 'ButtonPushedFcn', @(btn,event) wrap_backend(t, app, DataType, RegSel, Model, true));
            btn.Position = alpha*[750-145, 0, 135, 45];
            btn.Text = 'Export Cells';
            btn.BackgroundColor = app.bgclr;

            function wrap_backend(t, app, DataType, RegSel, Model, EXPcells)
                ...
                IO.func_ExportCSV_backend( ...
                    app, ...
                    path, ...
                    t.Data([t.Data{:,5}]==1, 6), ... Samples
                    t.Data([t.Data{:,1}]==1, 2), ... Phenotypes
                    t.Data([t.Data{:,3}]==1, 4), ... Channels
                    DataType.Value, ...
                    RegSel.Value, ...
                    Model.Value, ...
                    EXPcells ...
        ...
        function func_ExportCSV_backend(app, path, smpl, CellNames, ChNames, DataType, RegSel, Model, EXPcells)
            ...
            AllCl = ismember('All Cells', CellNames);
            CellNames = CellNames(~strcmp(CellNames, 'All Cells'));

            AllCh = ismember('All Channels', ChNames);
            ChNames = ChNames(~strcmp(ChNames, 'All Channels'));
            ...
            for smpl_i = 1:numel(smpl)
                ...
                if ~AllCh
                    dat = dat(:,unique(ChNames(ismember(ChNames, dat.Properties.VariableNames)), 'stable'));
                end
                
                % Write All Cells Table
                if AllCl
                    datSUB = dat;
                    if ~isempty(RegSel)
                        if numel(RegSel)==1
                    ...
                    writetable(datSUB,[strrep(smpl{smpl_i}, '/', '_') '.csv']);
                else
...
% % %             MFINMS = MFINMS(~startsWith(MFINMS, Constants.gate_tag));
% % % 
% % %             tbldat = Helper.populate_table(app, 'smpls', {app.DataN.Value});
% % % 
% % %             tbldat = [tbldat; tbldat(end,:)];
% % %             tbldat = [tbldat, tbldat(:,3:5)];
% % % 
% % %             tbldat(:, 1) = [];
% % %             tbldat(:, 1) = {false};
% % %             tbldat(:, 3) = [];
% % %             tbldat(1:numel(MFINMS) + 1, 3) = {false};
% % %             tbldat(1:numel(MFINMS), 4) = MFINMS;
% % %             tbldat(numel(MFINMS) + 1, 3) = {false};
            ...
            btn.Position = alpha*[550-100, 0, 100, 50];
            btn.Text = 'Ok';
            ...
            function wrap_backend(t, app)
                ...
                IO.func_ExportPrism_backend( ...
                    ...
                    t.Data([t.Data{:,5}]==1, 6), ... Samples
                    t.Data([t.Data{:,1}]==1, 2), ... Phenotypes
                    t.Data([t.Data{:,3}]==1, 4) ... Channel Names
                )
            end
        end
        ...
        function func_ExportPrism_backend(app, path, smpl, CellNames, ChNames)
            ...
            for smpl_i = 1:numel(smpl)
                ...
                % each sample
                GateNames = Helper.gate_full2tag(app,CellNames, smpl{smpl_i});
                dat = app.data.(smpl{smpl_i}).AllCells(:,ChNames);
                Gates = app.data.(smpl{smpl_i}).AllCells(:,GateNames);
                datMAT = zeros(max(sum(table2array(Gates))),numel(GateNames)*numel(ChNames));
                datMAT(datMAT==0) = NaN;
...
% %                             end
        ...
        function func_MergeSmpls(app, smpls, new_smpl_name)
