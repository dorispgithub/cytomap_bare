...
%#ok<*IMRECT>
    ...
    methods (Static)
        ...
        function func_newfig(app, varargin)
            ...
            toolbar = struct;
            toolbar.Main = findall(fig,'Type','uitoolbar');

            % Clear fugure
            toolbar.ClearFig  =  uipushtool('Parent',toolbar.Main);
            toolbar.ClearFig.TooltipString = 'Clear Plot';
            toolbar.ClearFig.CData = app.figOpts.icons.ClearFig;
            toolbar.ClearFig.Separator = 'on';
            toolbar.ClearFig.ClickedCallback = 'cla';

            % Update figure
            toolbar.UpdateFig  =  uipushtool('Parent',toolbar.Main);
            toolbar.UpdateFig.TooltipString = 'Update Plot';
            toolbar.UpdateFig.CData = app.figOpts.icons.RefreshPlot;
            toolbar.UpdateFig.ClickedCallback =  @(~,~) Plt_Helper.func_refresh(app, num);

            % Make a new rectangular gate
            toolbar.NewRectGate  =  uipushtool('Parent',toolbar.Main);
            toolbar.NewRectGate.TooltipString = 'New Rectangular Gate';
            toolbar.NewRectGate.CData = app.figOpts.icons.RectGate;
            toolbar.NewRectGate.Separator = 'on';
            toolbar.NewRectGate.ClickedCallback = @(~,~) Plt_Gating.new_gate(app, num, 'RectGate');

            % Make a new Polygon gate
            toolbar.NewRectGatePoly  =  uipushtool('Parent',toolbar.Main);
            toolbar.NewRectGatePoly.TooltipString = 'New Polygon Gate';
            toolbar.NewRectGatePoly.CData = app.figOpts.icons.PolyGate;
            toolbar.NewRectGatePoly.Separator = 'off';
            toolbar.NewRectGatePoly.ClickedCallback = @(~,~) Plt_Gating.new_gate(app, num, 'PolyGate');

            % Export Gated population
            toolbar.SaveGate = uipushtool('Parent',toolbar.Main);
            toolbar.SaveGate.TooltipString = 'Save Last Chosen Gate';
            toolbar.SaveGate.CData = app.figOpts.icons.Save;
            toolbar.SaveGate.Separator = 'off';
            toolbar.SaveGate.ClickedCallback = @(~,~) Plt_Gating.save_gate(app, num);

            % Axes even
            toolbar.eqax = uitoggletool('Parent',toolbar.Main);
            toolbar.eqax.TooltipString = 'Square Axes';
            toolbar.eqax.CData = app.figOpts.icons.EvenAxes;
            app.figOpts.eqax.(num) = true;
            toolbar.eqax.ClickedCallback = @(~,~) Plt_Helper.pltax(toolbar.eqax, app, num);
            toolbar.eqax.State= 'on';

            % Plot 3D
            toolbar.D3 = uitoggletool('Parent',toolbar.Main);
            toolbar.D3.TooltipString = 'Make plot 3D';
            toolbar.D3.CData = app.figOpts.icons.Plot3D;
            app.figOpts.D3.(num) = false;
            toolbar.D3.ClickedCallback = @(~,~) Plt_Helper.plt3D(toolbar.D3, app, num);
            toolbar.D3.State= 'off';

            % Invert Colors
            toolbar.IC = uitoggletool('Parent',toolbar.Main);
            toolbar.IC.TooltipString = 'Invert Colors';
            toolbar.IC.CData = app.figOpts.icons.INVERT;
            if strcmp(app.figOpts.bgclr.(num), 'w')
                app.figOpts.IC.(num)=false;
                toolbar.IC.State= 'off';
            else
                app.figOpts.IC.(num)=true;
                toolbar.IC.State= 'on';
            end
            toolbar.IC.ClickedCallback = @(~,~) Plt_Helper.pltIC(toolbar.IC, app, num);
            ...
            toolbar.UpdateFig  =  uipushtool('Parent',toolbar.Main);
            toolbar.UpdateFig.TooltipString = 'Rotate about Z axis';
            toolbar.UpdateFig.CData = app.figOpts.icons.Rotate;
            toolbar.UpdateFig.ClickedCallback =  @(~,~) Plt_Helper.frontend_rotate(app);
            % New Spots
            toolbar.PltSpts  =  uipushtool('Parent',toolbar.Main);
            toolbar.PltSpts.TooltipString = 'Draw Spots';
            toolbar.PltSpts.CData = app.figOpts.icons.NewPoints;
            toolbar.PltSpts.Separator = 'on';
            toolbar.PltSpts.ClickedCallback =  @(~, ~) Plt_Helper.func_Spots(app);
            % New Polygons
            toolbar.PltPoly  =  uipushtool('Parent',toolbar.Main);
            toolbar.PltPoly.TooltipString = 'Draw Polygons';
            toolbar.PltPoly.CData = app.figOpts.icons.NewPoly;
            toolbar.PltPoly.ClickedCallback =  @(~, ~) Plt_Helper.func_poly(app, '');
            % Plot Points
            toolbar.PltSpts  =  uipushtool('Parent',toolbar.Main);
            toolbar.PltSpts.TooltipString = 'Plot Spots';
            toolbar.PltSpts.CData = app.figOpts.icons.PlotPoints;
            toolbar.PltSpts.ClickedCallback =  @(~, ~) Plt_Helper.plotpts(app, num);
            % Make a surface around a group of points
            toolbar.VolWrap  =  uipushtool('Parent',toolbar.Main);
            toolbar.VolWrap.TooltipString = 'Make Surface';
            toolbar.VolWrap.CData = app.figOpts.icons.Wrap;
            toolbar.VolWrap.Separator = 'on';
            toolbar.VolWrap.ClickedCallback =  @(~, ~) Plt_Helper.frontend_surf(app, num, app.figOpts.smpls.(num), [], false, 'UDS');
            % Generate Random Points
            toolbar.VolWrap  =  uipushtool('Parent',toolbar.Main);
            toolbar.VolWrap.TooltipString = 'Generate Random Points';
            toolbar.VolWrap.CData = app.figOpts.icons.RandPoints;
            toolbar.VolWrap.Separator = 'off';
            toolbar.VolWrap.ClickedCallback =  @(~, ~) Plt_Helper.frontend_random_pts(app, num);
            ...
            app.figOpts.smplPhenoBtn.(num) = uicontrol('Style', 'pushbutton', ...
                'String', 'Show Table', ...
                'Position', [Constants.plt_menu_x0, fig.Position(4) - Constants.plt_menu_y0 - 30, 150, 40], ...
            ...
            app.figOpts.sample.(num).Value = find(strcmp(app.figOpts.sample.(num).String, app.figOpts.defaults.(num).S));
...
%             set(dcm_obj, 'UpdateFcn', {@myupdatefcn, app, num})
...
%                                     plot(smoothdata(X,'gaussian', 100), smoothdata(Y(IND_X),'gaussian', 100), '.', 'DisplayName', ['Smoothed_' dsnm]);
        ...
        function resize_figure(src, app)
            ...
            if strcmp(app.figOpts.smplPhenoBtn.(num).Visible, 'off')
                ...
                % Set axis to right hals of screen. Offsets are there to keep names visible
                ax.Position(1) = (app.figOpts.phn_smpl_tab.(num).Position(1) + app.figOpts.phn_smpl_tab.(num).Position(3) + 50.0) / src.Position(3);
                ax.Position(2) = (app.figOpts.sldrIF.(num).Position(2) + app.figOpts.sldrIF.(num).Position(4) + delta_y + 40.0) / src.Position(4);
                ax.Position(3) = 1 - ax.Position(1) - ((x_0 + app.figOpts.SldrZ.(num).Position(3) + 60.0) / src.Position(3));
                ...
                ax.Position(1) = (x_0 + 55) * 1.0 / src.Position(3);
                ax.Position(2) = (app.figOpts.sldrIF.(num).Position(2) + app.figOpts.sldrIF.(num).Position(4) + delta_y + 40.0) / src.Position(4);
                ax.Position(3) = 1 - ((2.0 * x_0 + 35. + app.figOpts.SldrZ.(num).Position(3)) / src.Position(3));
                ax.Position(4) = 1 - ax.Position(2) - ((y_0 + 55.0) / src.Position(4));
            end
