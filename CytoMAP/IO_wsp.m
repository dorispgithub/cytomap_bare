    ...
    methods (Static)
        ...
        function func_WSPload_backend(app, pnm, fnm)
            ...
            function [experiment_trees, tag_table, data, smpl2group_tab, sample_names] = load_wsp(file_name)
                ...
                sample_names = cell(numel(sample_idxs));
                for sample_index=1:numel(sample_idxs)
                    ...
                    data{sample_index} = get_fcs_dataset(file_name, sample); % Reads a fsj dataset in any of the subdirectories.
                ...
                function dataset = get_fcs_dataset(wsp_path, sample)
                    ...
                    dataset = insert_derived_channels(sample, dataset);
                end
                ...
                function tab = insert_derived_channels(sample, tab)
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    % Input:
                    ...
                    for dp_idx=1:numel(dps)
                        ...
                        tab.(name) = process_derived_formula(formula, tab);
                    end

                end

                function col = process_derived_formula(formula, tab)
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    for j=indexes
                        if strcmp(keywords.Children(j).Attributes(1).Value(1:2), '$P') && ...
                            ...
                            switch keywords.Children(j).Attributes(1).Value(end)
                                ...
                                case 'S'
% % %                                     % if Flowjo name ($P**N) comes after channel name
% % %                                     if isempty(chnl)
% % %                                         chnl = keywords.Children(j+2).Attributes(2).Value;
% % %                                     end
...
% % %                                         {key; value} % these seem to
% be correct pairs of parameter number and channel name CHECk that gatting
% is pulling the righ channels
                                        map(key) = value;
                                    end
% % %                                     % dump channel name
% % %                                     chnl = [];
