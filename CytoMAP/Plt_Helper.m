    ...
    methods (Static)
        ...
        function func_poly(app, name0, web)
            ...
            if isempty(name0)
                ...
                app.polygons.TMP.P1.pos = getPosition(POLY);
                app.polygons.TMP.P1.clr = getColor(POLY);
                ...
                for poly_i=1:numel(polysubnames)
                    ...
                    iPOLY = impoly(gca, POLY.pos);
                    setColor(iPOLY, POLY.clr);
                    app.polygons.TMP.(['P' num2str(poly_i)]).obj = iPOLY;
                    app.polygons.TMP.(['P' num2str(poly_i)]).pos = getPosition(iPOLY);
                    app.polygons.TMP.(['P' num2str(poly_i)]).clr = getColor(iPOLY);
                end
            end
        ...
        % % %     btn.Text = 'Export Population';
            ...
            function func_poly_add(~, app)
                ...
                app.polygons.TMP.(['P' num2str(Npoly)]).pos = getPosition(POLY2);
                app.polygons.TMP.(['P' num2str(Npoly)]).clr = getColor(POLY2);
            end
            ...
            function func_poly_save(t,app,UIfig)
                app.polygons.(t.Data{2,1}) = app.polygons.TMP;
...
%                 Plt_Helper.edit_colormap_backend(app, DataType.Value, cell2mat(t.Data(1:end,3:5)));
        ...
        function edit_colormap_backend(app, DataType, newORDER, rgb)
            ...
            % Make sure the user actually wants to override an existing
            % model
            if any(ismember(fieldnames(app.net), ModelName))
                answer = questdlg(['Are you certain you want to override' ModelName], ...
                    'Dessert Menu', ...
                    'Yes', 'No', 'No');
...
%                 channels = fieldnames(app.data.(smples{smp_i}).AllCells);
        ...
        function plot_options(app, num, web)
            ...
            app.figOpts.limits.(num)(1:4, 3) = {'X-Axis', 'Y-Axis', 'Z-Axis', 'Color-Axis'};
            app.figOpts.limits.(num)(:, 4) = {false};
            app.figOpts.limits.(num)(:, 5) = {false};
            ...
            UIfig.Position(3:4) = [350 200];
            % Create the table of options
            t = uitable(UIfig);
            t.Data = app.figOpts.limits.(num);
            t.Position = alpha*[0 40 350 160];
        ...
        function plot_options_backend(app, UIfig, num, automodes, ranges, scales)
...
%             tab_limits = cell(4, 5);
%             tab_limits(:, 3) = app.figOpts.limits.(num)(:, 3);
%             tab_limits(:, 1:2) = mat2cell(ranges, 2 * ones(size(ranges, 1)));
%             tab_limits(:, 4) = mat2cell(automodes, ones(automodes));
%             tab_limits(:, 5) = mat2cell(scales, ones(scales));
%             app.figOpts.limits.(num) = tab_limits;
%             delete(UIfig)
        end
...
%                 dlmwrite(file, datMAT', '-append', 'precision', 5)
                ...
                elseif strcmp(app.figOpts.caxIF.(num).String{app.figOpts.caxIF.(num).Value}, 'Number of cells / Neighborhood')
                    ...
                    X = PltDat(i).XData;
                    Y = PltDat(i).YData;
                    C = PltDat(i).CData'; % why does MATLAB switch formats...
                ...
                fclose(fid);
                dlmwrite(file,datMAT', '-append', 'precision', 5)                
% %                 %%% dat % When exporting spatial data this sometimes breaks for no reason
% %                 writetable(dat,file, 'WriteVariableNames', false);
            end
            cd(curr_path);
        end
