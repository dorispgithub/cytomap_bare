    ...
    methods(Static)
        ...
        function dist_func(app, web)
            ...
            function backend_wrap(app, t, UIfig, ponlyN, DensButton, Den, SurfN, ModelN, special_names)
                ...
                pheno(pheno) = ~ismember(t.Data(pheno, 2), special_names);
                pheno = t.Data(pheno, 2);
                Analysis.dist_func_backend( ...
                    app, ...
                    t.Data([t.Data{:, 3}]'==1, 4), ... Samples
                    pheno, ... Phenotypes
                    'calc_spots', logical(t.Data{strcmp(t.Data(:, 2), 'Spots'), 1}), ... Spots
                    'calc_poly', logical(t.Data{strcmp(t.Data(:, 2), 'Polygons'), 1}), ... Polygons
                    'calc_reg', logical(t.Data{strcmp(t.Data(:, 2), 'Regions'), 1}), ... Regions
                    'calc_surf', logical(t.Data{strcmp(t.Data(:, 2), 'User Defined Surfaces'), 1}), ... User Defined Surface
        ...
        function dist_func_backend(app, smplnms, pnms, varargin)
            ...
            for smp_i=1:numel(smplnms)
                ...
                if logical(defaults.calc_reg)
                    if ~isfield(app.data.(smplnms{smp_i}), 'MFIRSN') || ...
                            ~isfield(app.data.(smplnms{smp_i}).MFIRSN, defaults.model_name)
                        warndlg('No Raster Scanned Neighborhoods/Models with this name found. Skipping Regions option.', smplnms{smp_i});
                    else
                        ROWDat = app.data.(smplnms{smp_i}).MFIRSN.(defaults.model_name);
                        for l=0:max(ROWDat) % for each region
                            if ~isvalid(vPD)
                                return;
                            end
                            waitbar((l)/(max(ROWDat)), vPD, 'Calculating distances to Regions');
                            % Pull out the x and y data from the
                            % cellularity table corresponding to the
                            % current region
                            todat = app.data.(smplnms{smp_i}).MFIRSN(ROWDat==l, {'X', 'Y', 'Z'});
                            ...
                            for loopi = 1:Nloop
                                ...
                                app.data.(smplnms{smp_i}).AllCells.(Helper.valid_var([Constants.other_tag 'DistToReg_' num2str(l) 'Model_' model_name]))(st:en) = Dist;
                            end
                        end
                    end
                end
...
%%%%%
            ...
            for i = 1:numel(smplnms)
                ...
                pnms_sub = Helper.get_gate_tags(app);
                % Pull just the channel names
                chnms_sub = Helper.get_channels(app);
                ...
                MFICCN = zeros(size(selected_idxs, 1), (numel(pnms_sub)+numel(chnms_sub)+7));
                ...
                for batch_idx=1:n_batches
                    ...
                    IDIND = contains(ChNms, 'ChID') | contains(ChNms, 'ChchID')| contains(ChNms, 'chID');
                ...
                close(vPD);
                if warning
                    errordlg(sprintf( [smplnms{i} '\nData does not contain cell ID numbers. \nSome functions will not work']), 'Error');
                end
...
%                 return;
            ...
            % Select Neighborhood type
            lbl = uilabel(UIfig); lbl.Text = sprintf('Select Neighborhood:');
            Helper.func_SetCLR(app, lbl, 'label')
            ...
            t.ColumnName = {'Weight', 'Use for sorting', 'Phenotype (Must be in all samples)', ...
            ...
            DataType.ValueChangedFcn = @(btn, event) check_smpl(app, t, DataType);
            ...
            ClustAlgorithm.Items = {'NN Self Organizing Map', 'k-means','Gaussian Distribution Model'};
            ...
            NetName.ValueChangedFcn = @(dd, p) net_changed(dd, p, ShowNetBtn);
            ...
            % Create a push button
            btn = uibutton(UIfig, 'push', 'ButtonPushedFcn', @(btn,event) backend_wrap( ...
                t, app, ClrMap, DataPrep, DataType, ClustAlgorithm, NofClassesAlg, NetName, NormPer, DataPrepMFI));
            ...
            function backend_wrap(t, app, ClrMap, DataPrep, DataType, ClustAlgorithm, NofClassesAlg, NetName, NormPer, DataPrepMFI)
            ...
            function check_smpl(app, t, DataType)
            ...
            function show_train(app, netname, web)
                if nargin<3
                    web=0;
                end
                alpha=app.GUIOPTS.Alpha{app.GUIOPTS.AlphaI};
                ...
                if web==1
                    show_fig.Visible='OFF';
                end
                show_fig.Position = alpha*[10 10 1000 600];
                ...
                uitable(show_fig, 'Data', past_table, 'Position', alpha*[10, 10, 1000, 600]);
            end

            function net_changed(~, p, Btn)
        ...
        function TNN_backend(app, smplnms, phenotypes, pheno_weights, MFIList, MFI_weights, ClrMap, DataPrep, DataType, ClustAlgorithm, NofClassesAlg, ModelName, NormOpt, DataPrepMFI)
            ...
            %       'Raster Scanned Neighborhoods' or
            %       'Cell Centered Neighborhoods'
...
%%%%%
            ...
            if new_model
                ...
                switch NofClassesAlg
                    case {'Davies Bouldin (Default)', 'Davies Bouldin'}
                        NR = evalclusters(Dat_Pre, @(data, idx) gap_helper(idx, ClustAlgorithm, data), 'DaviesBouldin', 'KList', 1:24);
                        figure(fig)
                        plot(NR.CriterionValues, 'b-o');
                        fig.Visible = true;
                        xlabel('Number of Clusters');
                        ylabel('Davies-Bouldin values');
                        axis square
                        box off
                    ...
                    case {'Calinski-Harabasz (Default)', 'Calinski-Harabasz'}
                        NR = evalclusters(Dat_Pre, @(data, idx) gap_helper(idx, ClustAlgorithm, data), 'CalinskiHarabasz', 'KList', 1:24);
                        figure(fig)
                        plot(NR.CriterionValues, 'b-o');
                        fig.Visible = true;
                        xlabel('Number of Clusters');
                        ylabel('Calinski-Harabasz values');
                        axis square
                        box off
                        NRegCValues = NR.CriterionValues;
                        NR = NR.OptimalK;
                    ...
                    case {'Gap (Default)', 'Gap'}
                        NR = evalclusters(Dat_Pre, @(data, idx) gap_helper(idx, ClustAlgorithm, data), 'Gap', 'KList', 1:24);
                        figure(fig)
                        plot(NR.CriterionValues, 'b-o');
                        fig.Visible = true;
                        xlabel('Number of Clusters');
                    ...
                    otherwise % Manual Choice (Numerical)
...
%%%%%
            waitbar(0.15, vPD, 'Clustering Data');
                ...
                if strcmp(ClustAlgorithm, 'NN Self Organizing Map')
                    % Train a neural network to automatically classify neighborhoods
                    if ~isvalid(vPD)
                        return;
                    end
                    waitbar(0.2, vPD, 'Training Neural Network');

                    % Train a NEW neural network to automatically classify neighborhoods
                    % REUSE if it already existed
                    if new_model
                        app.net.(ModelName).type = 'SOM';
                        % Create a Self-Organizing Map
                        app.net.(ModelName).Network = selforgmap([NR 1]);
                        %Don't show the training window
                        app.net.(ModelName).Network.trainParam.showWindow = false;
                        % Train the Network
                        [app.net.(ModelName).Network, ~] = train(app.net.(ModelName).Network, Dat_Pre');
                    end

                    % Sort the local neighborhoods into regions with Neural Net
                    if ~isvalid(vPD)
                        return;
                    end
                    waitbar(0.5, vPD, 'Sorting networks');
                    ROWAllDat = vec2ind(app.net.(ModelName).Network(Dat_Pre'))';
                elseif strcmp(ClustAlgorithm, 'k-means')
                    % Sort with k-means. Create new net if needed
                    if new_model
                        app.net.(ModelName).type = 'k-means';
                    end
                    if ~isvalid(vPD)
                        return;
                    end
                    waitbar(0.2, vPD, 'Clustering with k-means');
                    ROWAllDat = kmeans(Dat_Pre,NR);
                elseif strcmp(ClustAlgorithm, 'Gaussian Distribution Model')
                    app.net.(ModelName).type = 'gauss dist';
                    % Generate a gaussian distribution model
                    gm = fitgmdist(Dat_Pre,NR,'RegularizationValue',0.1);
                    % I think this is wrong here TODO: fit it
                    % Implement hard clustering with the model
                    ROWAllDat = cluster(gm,Dat_Pre);
                end
                ...
                for i=1:numel(smplnms)
                    ...
                    % Pull the number of neighborhoods
...
% % %                         NNeighbor = size(app.data.(smplnms{i}).MFICCN, 1);
% % %                     end
                ...
                for i=1:numel(smplnms)
                    ...
                    if strcmp(DataType, 'Cell Centered Neighborhoods')
                        ...
                        [PositionsN, INDN] = unique(table2array(app.data.(smplnms{i}).MFICCN(:, {'X', 'Y', 'Z'})), 'rows');
                ...
                waitbar(0.95, vPD, 'Plotting regions');
                for i=1:numel(smplnms)
                    Class = ROW{i};
                    % 3D Data
                    ...
                    if strcmp(DataType, 'Raster Scanned Neighborhoods')
                        Plotting.func_newfig(app, 'S', smplnms{i}, 'C', ModelName);
                        fig = gcf;
                        cla
...
%                             title(Helper.full_var(smplnms{i}), 'Color', app.GUIOPTS.txtclr);
                        ...
                        c.Ticks = round(min(min(cell2mat(ROW)))):round(max(max(cell2mat(ROW))));
                        c.Limits = [round(min(min(cell2mat(ROW)))), round(max(max(cell2mat(ROW))))];
                    ...
                    elseif strcmp(DataType, 'Cell Centered Neighborhoods')
                        if false
                            ...
                            title(ttl{i}, 'Color', app.GUIOPTS.txtclr)
                            axis tight
                            axis equal
                            ax = gca;
                            ax.Color = app.GUIOPTS.bgclr;
...
%                                     surf(Xs*app.rwindowCCN+Position(j,1), Ys*app.rwindowCCN+Position(j,2), Zs*app.rwindowCCN+Position(j,3), 'Facecolor',app.map(Class(j)+1, :), 'EdgeColor', 'none','FaceAlpha',0.3);
                        ...
                        c = colorbar;
                        c.Color = app.GUIOPTS.txtclr;
                        c.Ticks = round(min(cell2mat(ROW))-1):round(max(cell2mat(ROW)));
                        c.Limits = [round(min(cell2mat(ROW))-1), round(max(cell2mat(ROW)))];
                        % Change the color scale to match the limits
                        caxis([round(min(cell2mat(ROW))-1), round(max(cell2mat(ROW)))])
                        ylabel(c, 'Region number')
                    end
                end % end of plots
...
%%%%%
            ...
            function result = gap_helper(n_cluster, alg, data)
                if strcmp(alg, 'NN Self Organizing Map')
                    net = selforgmap([n_cluster 1]);
                    %Don't show the training window
                    net.trainParam.showWindow = false;
                    % Train the Network
                    [net, ~] = train(net, data');
                    result = vec2ind(net(data'))';
                elseif strcmp(alg, 'k-means')
                    result = kmeans(data, n_cluster);
                elseif strcmp(alg, 'Gaussian Distribution Model')
                    % Generate a gaussian distribution model
                    % I think this is wrong here TODO: fit it
                    gauss_model = fitgmdist(data, n_cluster, 'RegularizationValue', 0.1);
                    % Implement hard clustering with the model
                    result = cluster(gauss_model, data);
                end
            end
        end
